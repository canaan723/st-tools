...
CONFIG_FILE="$HOME/.st_assistant.conf" # 用于保存用户备份偏好的配置文件
...
run_backup_interactive() {
    ...
    local exclude_params=(-x "*/.git/*" -x "*/_cache/*" -x "*.log" -x "*/backups/*"); zip -rq "$backup_zip_path" "${paths_to_backup[@]}" "${exclude_params[@]}"
...
main_data_management_menu() {
    while true; do clear; fn_print_header "SillyTavern 数据管理"; echo -e "      [1] ${GREEN}创建自定义备份${NC}"; echo -e "      [2] ${CYAN}数据迁移/恢复指南${NC}"; echo -e "      [3] ${RED}删除旧备份${NC}"; echo -e "      [0] ${CYAN}返回主菜单${NC}"; read -p "    请输入选项: " choice
...
fn_create_shortcut() {
    ...
    if ! grep -qF "$ALIAS_CMD" "$BASHRC_FILE"; then
        chmod +x "$SCRIPT_SELF_PATH" # 关键：授予脚本执行权限
