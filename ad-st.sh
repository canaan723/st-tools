...
fn_run_npm_install_with_retry() {
    ...
    npm config delete registry; local exit_code; npm install --no-audit --no-fund --omit=dev; exit_code=$?
...
run_backup_interactive() {
        ...
        case "$user_choice" in
            ...
            *)
                if [[ "$user_choice" =~ ^[0-9]+$ ]] && [ "$user_choice" -ge 1 ] && [ "$user_choice" -le "${#options[@]}" ]; then
                    local selected_key="${options[$((user_choice-1))]}"; if ${selection_status[$selected_key]}; then selection_status[$selected_key]=false; else selection_status[$selected_key]=true; fi
                else fn_print_warning "无效输入。"; sleep 1; fi
                ;;
        esac
    done
    ...
    zip -rq "$backup_zip_path" "${paths_to_backup[@]}" "${exclude_params[@]}"; if [ $? -ne 0 ]; then fn_print_warning "备份失败！"; fn_press_any_key; return; fi
...
run_delete_backup() {
    ...
    mkdir -p "$BACKUP_ROOT_DIR"; mapfile -t backup_files < <(find "$BACKUP_ROOT_DIR" -maxdepth 1 -name "*.zip" -printf "%T@ %p\n" | sort -nr | cut -d' ' -f2-); if [ ${#backup_files[@]} -eq 0 ]; then fn_print_warning "未找到任何备份文件。"; fn_press_any_key; return; fi
    echo -e "检测到以下备份 (当前/上限: ${#backup_files[@]}/${BACKUP_LIMIT}):"
    local i=0; for file in "${backup_files[@]}"; do if [ -f "$file" ]; then local size; size=$(du -h "$file" | cut -f1); printf "    [%-2d] %-40s (%s)\n" "$((++i))" "$(basename "$file")" "$size"; fi; done
    ...
    if [[ "$confirm" =~ ^[yY]$ ]]; then rm "$chosen_backup"; fn_print_success "备份已删除。"; else echo "操作已取消。"; fi
    fn_press_any_key
}
...
# 助手脚本自我更新
main_update_script() {
    clear; fn_print_header "更新助手脚本"; echo -e "${YELLOW}正在从 Gitee 下载新版本...${NC}"; local temp_file=$(mktemp)
...
main_manage_autostart() {
    ...
    if [[ "$1" == "set_default" ]]; then
        if ! $is_set; then echo -e "\n# SillyTavern 助手\n$AUTOSTART_CMD" >> "$BASHRC_FILE"; fn_print_success "已设置 Termux 启动时自动运行本助手。"; fi; return
    fi
    clear; fn_print_header "管理助手自启"
    if $is_set; then
        echo -e "当前状态: ${GREEN}已启用${NC}"; echo -e "${CYAN}提示: 关闭自启后，输入 'st' 命令即可手动启动助手。${NC}"; read -p "是否取消自启？ (y/n): " confirm
        if [[ "$confirm" =~ ^[yY]$ ]]; then sed -i "/# SillyTavern 助手/d" "$BASHRC_FILE"; sed -i "\|$AUTOSTART_CMD|d" "$BASHRC_FILE"; fn_print_success "已取消自启。"; fi
    else
        echo -e "当前状态: ${RED}未启用${NC}"; echo -e "${CYAN}提示: 在 Termux 中输入 'st' 命令可以手动启动助手。${NC}"; read -p "是否设置自启？ (y/n): " confirm
        if [[ "$confirm" =~ ^[yY]$ ]]; then echo -e "\n# SillyTavern 助手\n$AUTOSTART_CMD" >> "$BASHRC_FILE"; fn_print_success "已成功设置自启。"; fi
    fi
    fn_press_any_key
}
...
# 显示并尝试打开在线帮助文档
main_open_docs() {
    clear; fn_print_header "查看帮助文档"; local docs_url="https://stdocs.723123.xyz"; echo -e "文档网址: ${CYAN}${docs_url}${NC}\n"
