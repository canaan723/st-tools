...
# SillyTavern 助手 v2.3 (错误修正稳定版)
# 作者: Qingjue | 小红书号: 826702880
# v2.3: 修复了镜像测速输出污染和备份菜单无法勾选的严重 BUG。
...
# [修复] 将所有提示信息重定向到 stderr (>&2)，避免污染 stdout
...
main_start() {
    ...
    echo -e "${YELLOW}环境准备就绪，正在启动SillyTavern服务...${NC}"; echo -e "${YELLOW}首次启动或更新后会自动安装依赖，耗时可能较长，请耐心等待...${NC}"
    bash start.sh
...
main_install() {
    ...
    fn_print_header "1/5: 配置软件源"; echo -e "${YELLOW}即将开始配置 Termux 软件源...${NC}"; echo -e "  - 稍后会弹出一个蓝白色窗口，请根据提示操作。"; echo -e "  - ${GREEN}推荐：${NC}依次选择 ${BOLD}第一项${NC} -> ${BOLD}第三项${NC} (国内最优)。"; echo -e "\n${CYAN}请按任意键以继续...${NC}"; read -n 1 -s; termux-change-repo
    fn_print_header "2/5: 安装核心依赖"; echo -e "${YELLOW}正在更新软件包列表并安装核心依赖...${NC}"; yes | pkg update && yes | pkg upgrade || fn_print_error_exit "软件源更新失败！"; yes | pkg install git nodejs-lts rsync zip termux-api coreutils gawk bc || fn_print_error_exit "核心依赖安装失败！"; fn_print_success "核心依赖安装完毕。"
    ...
    if [ -f "$ST_DIR/start.sh" ]; then fn_print_warning "检测到完整的 SillyTavern 安装，跳过下载。"; elif [ -d "$ST_DIR" ] && [ -n "$(ls -A "$ST_DIR")" ]; then fn_print_error_exit "目录 $ST_DIR 已存在但安装不完整。请手动删除该目录后再试。"; else
        ...
        if [ ${#sorted_mirrors[@]} -eq 0 ]; then fn_print_error_exit "所有镜像均测试失败，无法下载主程序。"; fi
        local download_success=false
        for mirror_url in "${sorted_mirrors[@]}"; do
            local mirror_host; mirror_host=$(echo "$mirror_url" | sed -e 's|https://||' -e 's|/.*$||')
            fn_print_warning "正在尝试从镜像 [${mirror_host}] 下载 (${REPO_BRANCH} 分支)..."
            if git clone --depth 1 -b "$REPO_BRANCH" "$mirror_url" "$ST_DIR"; then fn_print_success "主程序下载完成。"; download_success=true; break; else fn_print_error "使用镜像 [${mirror_host}] 下载失败！正在切换下一条线路..."; rm -rf "$ST_DIR"; fi
        done
...
main_update_st() {
    ...
    mapfile -t sorted_mirrors < <(fn_find_fastest_mirror)
    if [ ${#sorted_mirrors[@]} -eq 0 ]; then fn_print_error_exit "所有镜像均测试失败，无法更新。"; fi
    ...
    for mirror_url in "${sorted_mirrors[@]}"; do
        ...
        if [ $? -eq 0 ]; then
            ...
            if echo "$git_output" | grep -qE "overwritten by merge|Please commit|unmerged files"; then
                clear; fn_print_header "检测到更新冲突！"; fn_print_warning "原因: 你可能修改过酒馆的文件，导致无法自动合并。"; echo "--- 冲突文件预览 ---"; echo "$git_output" | grep -E "^\s+" | head -n 5; echo "--------------------"
...
main_update_script() {
    ...
    if ! curl -L -o "$temp_file" "$SCRIPT_URL"; then rm -f "$temp_file"; fn_print_warning "下载失败，请检查网络。"; elif cmp -s "$SCRIPT_SELF_PATH" "$temp_file"; then rm -f "$temp_file"; fn_print_success "当前已是最新版本。"; else
...
run_backup_interactive() {
    ...
    if [ ! -f "$ST_DIR/start.sh" ]; then fn_print_warning "SillyTavern 尚未安装。"; fn_press_any_key; return; fi
    cd "$ST_DIR" || fn_print_error_exit "无法进入 SillyTavern 目录: $ST_DIR"
