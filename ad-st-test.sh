...
BACKUP_ROOT_DIR="$HOME/SillyTavern_Backups"                                   # 备份文件的存放目录 (移至ST同级，更安全)
...
fn_find_fastest_mirror() {
    if [ ${#CACHED_MIRRORS[@]} -gt 0 ]; then fn_print_success "已使用缓存的测速结果。" >&2; printf '%s\n' "${CACHED_MIRRORS[@]}"; return 0; fi
    fn_print_warning "开始测试 Git 镜像连通性与速度..." >&2; local github_url="https://github.com/SillyTavern/SillyTavern.git"; local temp_sorted_list=()
    if [[ " ${MIRROR_LIST[*]} " =~ " ${github_url} " ]]; then
        echo -e "  [1/?] 正在优先测试 GitHub 官方源..." >&2
        if timeout 15s git ls-remote "$github_url" HEAD >/dev/null 2>&1; then fn_print_success "GitHub 官方源直连可用，将优先使用！" >&2; temp_sorted_list=("$github_url"); else fn_print_error "GitHub 官方源连接超时，将测试其他镜像..." >&2; fi
    fi
    ...
    echo -e "${YELLOW}已启动并行测试，等待所有镜像响应...${NC}" >&2; local results_file; results_file=$(mktemp); local pids=()
    for mirror_url in "${other_mirrors[@]}"; do ( local mirror_host=$(echo "$mirror_url" | sed -e 's|https://||' -e 's|/.*$||'); local start_time=$(date +%s.%N); if timeout 15s git ls-remote "$mirror_url" HEAD >/dev/null 2>&1; then local end_time=$(date +%s.%N); local elapsed_time=$(echo "$end_time - $start_time" | bc); echo "$elapsed_time $mirror_url" >> "$results_file"; echo -e "  [${GREEN}✓${NC}] 测试成功: ${CYAN}${mirror_host}${NC} - 耗时 ${GREEN}${elapsed_time}s${NC}" >&2; else echo -e "  [${RED}✗${NC}] 测试失败: ${CYAN}${mirror_host}${NC} - ${RED}连接超时或无效${NC}" >&2; fi ) & pids+=($!); done
    wait "${pids[@]}"; if [ ! -s "$results_file" ]; then fn_print_error "所有镜像都无法连接。" >&2; rm -f "$results_file"; return 1; fi
    local fastest_line=$(sort -n "$results_file" | head -n 1); fn_print_success "已选定最快镜像: $(echo "$fastest_line" | awk '{print $2}' | sed -e 's|https://||' -e 's|/.*$||') (耗时 $(echo "$fastest_line" | awk '{print $1}')s)" >&2
    mapfile -t temp_sorted_list < <(sort -n "$results_file" | awk '{print $2}'); rm -f "$results_file"
...
fn_run_npm_install_with_retry() {
    ...
    fn_print_warning "国内镜像安装失败，将切换到NPM官方源进行最后尝试..."; npm config delete registry; local exit_code; npm install --no-audit --no-fund --omit=dev; exit_code=$?
...
fn_update_source_with_retry() {
    fn_print_header "1/5: 配置软件源"; echo -e "${YELLOW}即将开始配置 Termux 软件源...${NC}\n  - 稍后会弹出一个蓝白色窗口，请根据提示操作。\n  - ${GREEN}推荐：${NC}依次选择 ${BOLD}第一项${NC} -> ${BOLD}第三项${NC} (国内最优)。\n\n${CYAN}请按任意键以继续...${NC}"; read -n 1 -s
    for i in {1..3}; do termux-change-repo; fn_print_warning "正在更新软件包列表 (第 $i/3 次尝试)..."; if pkg update -y; then fn_print_success "软件源配置并更新成功！"; return 0; fi; if [ $i -lt 3 ]; then fn_print_error "当前选择的镜像源似乎有问题，正在尝试自动切换..."; sleep 2; fi; done
...
fn_create_data_zip_backup() {
    ...
    if zip -rq "$backup_zip_path" "${paths_to_backup[@]}" "${exclude_params[@]}"; then
        ...
        cd "$HOME"
        echo "$backup_zip_path" # 返回成功的备份路径
        return 0
    else
...
main_install() {
    ...
    if [[ "$1" == "no-start" ]]; then auto_start=false; fi
    ...
    if [[ "$auto_start" == "true" ]]; then
        ...
        fn_print_header "2/5: 安装核心依赖"; echo -e "${YELLOW}正在安装核心依赖...${NC}"; yes | pkg upgrade -y; yes | pkg install git nodejs-lts rsync zip unzip termux-api coreutils gawk bc || fn_print_error_exit "核心依赖安装失败！"; fn_print_success "核心依赖安装完毕。"
    fi
...
main_update_st() {
    ...
    while ! $update_success; do
        ...
        for mirror_url in "${sorted_mirrors[@]}"; do
            local mirror_host=$(echo "$mirror_url" | sed -e 's|https://||' -e 's|/.*$||'); fn_print_warning "正在尝试使用镜像 [${mirror_host}] 更新..."; git remote set-url origin "$mirror_url"
            ...
            if [ $? -eq 0 ]; then fn_print_success "代码更新成功。"; if fn_run_npm_install_with_retry; then update_success=true; fi; break; else
                if echo "$git_output" | grep -qE "overwritten by merge|Please commit|unmerged files"; then
                    ...
                    case "$choice" in
                        ""|'b'|'B')
                            clear; fn_print_header "步骤 1/5: 创建核心数据备份"
                            local data_backup_zip_path; data_backup_zip_path=$(fn_create_data_zip_backup)
                            if [ -z "$data_backup_zip_path" ]; then fn_print_error_exit "核心数据备份(.zip)创建失败，更新流程终止。"; fi

                            fn_print_header "步骤 2/5: 完整备份当前目录"
                            local renamed_backup_dir="${ST_DIR}_backup_$(date +%Y%m%d%H%M%S)"
                            cd "$HOME"; mv "$ST_DIR" "$renamed_backup_dir" || fn_print_error_exit "备份失败！请检查权限或手动重命名后重试。"
                            fn_print_success "旧目录已完整备份为: $(basename "$renamed_backup_dir")"

                            fn_print_header "步骤 3/5: 下载并安装新版 SillyTavern"
                            main_install "no-start"
                            if [ ! -d "$ST_DIR" ]; then fn_print_error_exit "新版本安装失败，流程终止。"; fi

                            fn_print_header "步骤 4/5: 自动恢复用户数据"
                            fn_print_warning "正在将备份数据解压至新目录..."
                            if ! unzip -o "$data_backup_zip_path" -d "$ST_DIR" >/dev/null 2>&1; then fn_print_error_exit "数据恢复失败！请检查zip文件是否有效。"; fi
                            ...
                            echo -e "\n${CYAN}请按任意键，启动更新后的 SillyTavern...${NC}"; read -n 1 -s
                            main_start; return
                            ;;
                        '1')
...
# ... [其他未修改的函数] ...
run_backup_interactive() { clear; fn_print_header "创建自定义备份"; if [ ! -f "$ST_DIR/start.sh" ]; then fn_print_warning "SillyTavern 尚未安装，无法备份。"; fn_press_any_key; return; fi; cd "$ST_DIR" || fn_print_error_exit "无法进入 SillyTavern 目录: $ST_DIR"; declare -A ALL_PATHS=(["./data"]="用户数据 (聊天/角色/设置)" ["./public/scripts/extensions/third-party"]="前端扩展" ["./plugins"]="后端扩展" ["./config.yaml"]="服务器配置 (网络/安全)"); local options=("./data" "./public/scripts/extensions/third-party" "./plugins" "./config.yaml"); local default_selection=("./data" "./plugins" "./public/scripts/extensions/third-party"); local selection_to_load=(); if [ -f "$CONFIG_FILE" ]; then mapfile -t selection_to_load < "$CONFIG_FILE"; fi; if [ ${#selection_to_load[@]} -eq 0 ]; then selection_to_load=("${default_selection[@]}"); fi; declare -A selection_status; for key in "${options[@]}"; do selection_status["$key"]=false; done; for key in "${selection_to_load[@]}"; do if [[ -v selection_status["$key"] ]]; then selection_status["$key"]=true; fi; done; while true; do clear; fn_print_header "请选择要备份的内容"; echo "输入数字可切换勾选状态。"; for i in "${!options[@]}"; do local key="${options[$i]}"; local description="${ALL_PATHS[$key]}"; if ${selection_status[$key]}; then printf "  [%-2d] ${GREEN}[✓] %s${NC}\n" "$((i+1))" "$key"; else printf "  [%-2d] [ ] %s${NC}\n" "$((i+1))" "$key"; fi; printf "      ${CYAN}(%s)${NC}\n" "$description"; done; echo -e "      ${GREEN}[回车] 开始备份${NC}      ${RED}[0] 取消备份${NC}"; read -p "请操作 [输入数字, 回车 或 0]: " user_choice; case "$user_choice" in "" | [sS]) break ;; 0) echo "备份已取消。"; fn_press_any_key; return ;; *) if [[ "$user_choice" =~ ^[0-9]+$ ]] && [ "$user_choice" -ge 1 ] && [ "$user_choice" -le "${#options[@]}" ]; then local selected_key="${options[$((user_choice-1))]}"; if ${selection_status[$selected_key]}; then selection_status[$selected_key]=false; else selection_status[$selected_key]=true; fi; else fn_print_warning "无效输入。"; sleep 1; fi ;; esac; done; local paths_to_backup=(); for key in "${options[@]}"; do if ${selection_status[$key]}; then if [ -e "$key" ]; then paths_to_backup+=("$key"); else fn_print_warning "路径 '$key' 不存在，已跳过。"; fi; fi; done; if [ ${#paths_to_backup[@]} -eq 0 ]; then fn_print_warning "您没有选择任何有效的项目，备份已取消。"; fn_press_any_key; return; fi; mkdir -p "$BACKUP_ROOT_DIR"; local timestamp; timestamp=$(date +"%Y-%m-%d_%H-%M"); local backup_name="ST_备份_${timestamp}"; local backup_zip_path="${BACKUP_ROOT_DIR}/${backup_name}.zip"; echo -e "\n${YELLOW}正在根据您的选择压缩文件...${NC}"; echo "包含项目:"; for item in "${paths_to_backup[@]}"; do echo "  - $item"; done; local exclude_params=(-x "*/.git/*" -x "*/_cache/*" -x "*.log" -x "*/backups/*"); zip -rq "$backup_zip_path" "${paths_to_backup[@]}" "${exclude_params[@]}"; if [ $? -ne 0 ]; then fn_print_warning "备份失败！"; fn_press_any_key; return; fi; mapfile -t all_backups < <(find "$BACKUP_ROOT_DIR" -maxdepth 1 -name "*.zip" -printf "%T@ %p\n" | sort -nr | cut -d' ' -f2-); fn_print_success "备份成功：${backup_name}.zip (当前备份数: ${#all_backups[@]}/${BACKUP_LIMIT})"; printf "%s\n" "${paths_to_backup[@]}" > "$CONFIG_FILE"; if [ "${#all_backups[@]}" -gt $BACKUP_LIMIT ]; then echo -e "${YELLOW}备份数量超过上限，正在清理旧备份...${NC}"; local backups_to_delete=("${all_backups[@]:$BACKUP_LIMIT}"); for old_backup in "${backups_to_delete[@]}"; do rm "$old_backup"; echo "  - 已删除: $(basename "$old_backup")"; done; fn_print_success "清理完成。"; fi; fn_press_any_key; }
main_migration_guide() { clear; fn_print_header "数据迁移 / 恢复指南"; echo -e "${YELLOW}请遵循以下步骤，将您的数据从旧设备迁移到新设备，或恢复备份：${NC}"; echo -e "  1. 在旧设备上，用MT管理器等文件工具，进入目录：\n     ${CYAN}${BACKUP_ROOT_DIR}/${NC}"; echo -e "  2. 找到需要的备份压缩包 (例如 ST_核心数据_xxxx.zip)，并将其发送到新设备。"; echo -e "  3. 在新设备上，将这个压缩包移动或复制到 SillyTavern 的根目录：\n     ${CYAN}${ST_DIR}/${NC}"; echo -e "  4. 使用 MT 管理器等工具，将压缩包 ${GREEN}“解压到当前目录”${NC}。"; echo -e "  5. 如果提示文件已存在，请选择 ${YELLOW}“全部覆盖”${NC}。"; echo -e "  6. 操作完成后，重启 SillyTavern 即可看到所有数据。"; echo -e "\n${YELLOW}如需更详细的图文教程，请在主菜单选择 [7] 查看帮助文档。${NC}"; fn_press_any_key; }
run_delete_backup() { clear; fn_print_header "删除旧备份"; if [ ! -f "$ST_DIR/start.sh" ]; then fn_print_warning "SillyTavern 尚未安装，没有可管理的备份。"; fn_press_any_key; return; fi; mkdir -p "$BACKUP_ROOT_DIR"; mapfile -t backup_files < <(find "$BACKUP_ROOT_DIR" -maxdepth 1 -name "*.zip" -printf "%T@ %p\n" | sort -nr | cut -d' ' -f2-); if [ ${#backup_files[@]} -eq 0 ]; then fn_print_warning "未找到任何备份文件。"; fn_press_any_key; return; fi; echo -e "检测到以下备份 (当前/上限: ${#backup_files[@]}/${BACKUP_LIMIT}):"; local i=0; for file in "${backup_files[@]}"; do if [ -f "$file" ]; then local size; size=$(du -h "$file" | cut -f1); printf "    [%-2d] %-40s (%s)\n" "$((++i))" "$(basename "$file")" "$size"; fi; done; read -p "输入要删除的备份编号 (其他键取消): " choice; if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 1 ] || [ "$choice" -gt "${#backup_files[@]}" ]; then echo "操作已取消."; fn_press_any_key; return; fi; local chosen_backup="${backup_files[$((choice-1))]}"; read -p "确认删除 '$(basename "$chosen_backup")' 吗？(y/n): " confirm; if [[ "$confirm" =~ ^[yY]$ ]]; then rm "$chosen_backup"; fn_print_success "备份已删除。"; else echo "操作已取消。"; fi; fn_press_any_key; }
main_data_management_menu() { while true; do clear; fn_print_header "SillyTavern 数据管理"; echo -e "      [1] ${GREEN}创建自定义备份${NC}\n      [2] ${CYAN}数据迁移/恢复指南${NC}\n      [3] ${RED}删除旧备份${NC}\n      [0] ${CYAN}返回主菜单${NC}"; read -p "    请输入选项: " choice; case $choice in 1) run_backup_interactive ;; 2) main_migration_guide ;; 3) run_delete_backup ;; 0) break ;; *) echo -e "${RED}无效输入。${NC}"; sleep 1 ;; esac; done; }
main_update_script() { clear; fn_print_header "更新助手脚本"; echo -e "${YELLOW}正在从 Gitee 下载新版本...${NC}"; local temp_file=$(mktemp); if ! curl -L -o "$temp_file" "$SCRIPT_URL"; then rm -f "$temp_file"; fn_print_warning "下载失败。"; elif cmp -s "$SCRIPT_SELF_PATH" "$temp_file"; then rm -f "$temp_file"; fn_print_success "当前已是最新版本。"; else sed -i 's/\r$//' "$temp_file"; chmod +x "$temp_file"; mv "$temp_file" "$SCRIPT_SELF_PATH"; rm -f "$UPDATE_FLAG_FILE"; echo -e "${GREEN}助手更新成功！正在自动重启...${NC}"; sleep 2; exec "$SCRIPT_SELF_PATH" --updated; fi; fn_press_any_key; }
...
EOF
    ...
    case $choice in
        ...
        5) main_update_script ;; 6) main_manage_autostart ;; 7) main_open_docs ;;
