...
# SillyTavern 助手 v2.1 (回归初心稳定版)
# 作者: Qingjue | 小红书号: 826702880
# v2.1: 诚恳致歉并修正。恢复了所有 Termux 原有功能(如自启管理)，移除了错误的全局依赖检查。
...
fn_find_fastest_mirror() {
    ...
    if [ ! -s "$results_file" ]; then fn_print_error "所有镜像都无法连接。"; rm -f "$results_file"; return 1; fi
...
main_install() {
    ...
    fn_create_shortcut
    main_manage_autostart "set_default" # 恢复您设计的默认设置自启
            ...
            if echo "$git_output" | grep -qE "overwritten by merge|Please commit|unmerged files"; then
                ...
                case "$choice" in
                    ""|'b'|'B')
                        ...
                        mv "$ST_DIR" "$backup_dir" || fn_print_error_exit "备份失败！"
                        fn_print_success "备份完成！现在将开始重新下载全新版本..."
                        sleep 2
                        # 注意：此处调用 main_install 会再次引导换源，这是一个小瑕疵但保证了流程的完整和独立。
                        # 在当前脚本结构下，这是最稳妥的做法。
                        main_install
                        return
                        ;;
...
main_update_script() {
    clear; fn_print_header "更新助手脚本"; echo -e "${YELLOW}正在从 Gitee 下载新版本...${NC}"
