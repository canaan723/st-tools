...
# SillyTavern 助手 v2.1.0 (社区修正版)
# 作者: Qingjue | 小红书号: 826702880
# 重大功能新增：
# 1. 【新增】集成 Rclone (S3) 同步方案，作为Git方案之外的第二个选择。
# 2. Rclone方案支持版本化备份、交互式恢复和自动清理旧备份。
# 3. 所有配置流程均简化为纯粹的问答模式，移除指导性文字。
# 4. 修复并固化了Git方案的所有已知问题，包括测试逻辑和重试机制。
...
RCLONE_SYNC_CONFIG_FILE="$HOME/.st_rclone.conf"
...
git_sync_test_one_mirror_push() {
    ...
    (
        ...
        touch testfile.txt; git add testfile.txt; git commit -m "Sync test commit" -q
...
git_sync_find_pushable_mirror() {
    ...
    if [[ -z "$REPO_URL" || -z "$REPO_TOKEN" ]]; then fn_print_error "Git同步配置不完整或不存在。" >&2; return 1; fi
    ...
    if [[ " ${PULL_MIRROR_LIST[*]} " =~ " ${github_public_url} " ]]; then
        ...
        if git_sync_test_one_mirror_push "$official_url"; then
            echo -e "    ${GREEN}[成功]${NC}" >&2; successful_urls+=("$official_url")
        else
            echo -e "    ${RED}[失败]${NC}" >&2
        fi
    fi
    ...
    if [ ${#other_mirrors[@]} -gt 0 ]; then
        ...
        for mirror_url in "${other_mirrors[@]}"; do
            (
                local authed_push_url=""; local mirror_host; mirror_host=$(echo "$mirror_url" | sed -e 's|https://||' -e 's|/.*$||')
                if [[ "$mirror_url" == *"hub.gitmirror.com"* ]]; then authed_push_url="https://${REPO_TOKEN}@${mirror_host}/${repo_path}"
                elif [[ "$mirror_url" == *"/gh/"* ]]; then authed_push_url="https://${REPO_TOKEN}@${mirror_host}/gh/${repo_path}"
                elif [[ "$mirror_url" == *"/github.com/"* ]]; then authed_push_url="https://${REPO_TOKEN}@${mirror_host}/github.com/${repo_path}"
                else exit 1; fi
    ...
    if [ ${#successful_urls[@]} -gt 0 ]; then
        ...
        fn_print_error "所有上传线路均测试失败。" >&2; return 1
    fi
}

git_sync_backup_to_cloud() {
    clear; fn_print_header "Git备份数据到云端 (上传)"
    if ! git_sync_check_deps; then fn_press_any_key; return; fi
    if [ ! -f "$GIT_SYNC_CONFIG_FILE" ]; then fn_print_error "请先在菜单 [1] 中配置Git同步服务。"; fn_press_any_key; return; fi

    mapfile -t push_urls < <(git_sync_find_pushable_mirror)
    if [ ${#push_urls[@]} -eq 0 ]; then fn_print_error "未能找到任何支持上传的线路。"; fn_press_any_key; return; fi

    local backup_success=false
    for push_url in "${push_urls[@]}"; do
        local chosen_host; chosen_host=$(echo "$push_url" | sed -e 's|https://.*@||' -e 's|/.*$||')
...
git_sync_restore_from_cloud() {
    clear; fn_print_header "Git从云端恢复数据 (下载)"
    if ! git_sync_check_deps; then fn_press_any_key; return; fi
    if [ ! -f "$GIT_SYNC_CONFIG_FILE" ]; then fn_print_error "请先在菜单 [1] 中配置Git同步服务。"; fn_press_any_key; return; fi
    ...
    for pull_url in "${pull_urls[@]}"; do
        local temp_dir; temp_dir=$(mktemp -d); local chosen_host; chosen_host=$(echo "$pull_url" | sed -e 's|https://||' -e 's|/.*$||')
        fn_print_warning "正在尝试使用线路 [${chosen_host}] 进行恢复..."
        cd "$HOME" || { fn_print_error "无法进入家目录！"; rm -rf "$temp_dir"; fn_press_any_key; return; }
        # shellcheck source=/dev/null
        source "$GIT_SYNC_CONFIG_FILE"; local repo_path; repo_path=$(echo "$REPO_URL" | sed 's|https://github.com/||')
    ...
    if ! $restore_success; then fn_print_error "已尝试所有可用线路，但恢复均失败。"; fi
    fn_press_any_key
}
...
git_sync_clear_config() {
    if [ -f "$GIT_SYNC_CONFIG_FILE" ]; then
        ...
        if [[ "$confirm" =~ ^[yY]$ ]]; then rm -f "$GIT_SYNC_CONFIG_FILE"; fn_print_success "Git同步配置已清除。"; else fn_print_warning "操作已取消。"; fi
    else fn_print_warning "未找到任何Git同步配置。"; fi
    fn_press_any_key
}
...
menu_git_sync() {
    ...
    while true; do
        ...
        echo -e "      [1] ${CYAN}配置Git同步服务${NC}"; echo -e "      [2] ${GREEN}备份到云端 (上传)${NC}"; echo -e "      [3] ${YELLOW}从云端恢复 (下载)${NC}"; echo -e "      [4] ${RED}清除Git同步配置${NC}"; echo -e "      [0] ${CYAN}返回上一级${NC}"
        read -p "    请输入选项: " choice
        case $choice in 1) git_sync_configure ;; 2) git_sync_backup_to_cloud ;; 3) git_sync_restore_from_cloud ;; 4) git_sync_clear_config ;; 0) break ;; *) fn_print_error "无效输入。"; sleep 1 ;; esac
    done
}
...
rclone_check_deps() {
    if ! fn_check_command "rclone"; then fn_print_error "缺少核心工具 rclone。"; fn_print_warning "请先运行 [首次部署] 来安装所有必需的依赖项。"; return 1; fi
    return 0
}

rclone_configure() {
    clear; fn_print_header "配置 Rclone (S3) 同步服务"
    local remote_name="st-sync"
    local provider access_key secret_key endpoint bucket region
    echo "请选择您的S3服务商:"
    echo "  [1] Cloudflare R2 (推荐，有免费额度)"
    echo "  [2] 其他S3兼容服务商"
    read -p "请输入选项: " provider_choice
    case $provider_choice in
        1) provider="Cloudflare" ;;
        2) provider="Other" ;;
        *) fn_print_error "无效选择。"; fn_press_any_key; return ;;
    esac
    ...
    rclone config create \
        ...
        ${region:+region="$region"} >/dev/null 2>&1
    
    if [ $? -eq 0 ]; then
        echo "RCLONE_REMOTE_NAME=\"$remote_name\"" > "$RCLONE_SYNC_CONFIG_FILE"
        echo "RCLONE_BUCKET_NAME=\"$bucket\"" >> "$RCLONE_SYNC_CONFIG_FILE"
        fn_print_success "Rclone同步服务配置已保存！"
    else
        fn_print_error "Rclone配置创建失败！请检查输入信息。"
    fi
    fn_press_any_key
}

rclone_backup() {
    clear; fn_print_header "Rclone备份数据到云端 (上传)"
    if [ ! -f "$RCLONE_SYNC_CONFIG_FILE" ]; then fn_print_error "请先在菜单 [1] 中配置Rclone同步服务。"; fn_press_any_key; return; fi
    # shellcheck source=/dev/null
    source "$RCLONE_SYNC_CONFIG_FILE"

    local timestamp; timestamp=$(date +"%Y-%m-%d_%H-%M-%S")
    local remote_path="${RCLONE_REMOTE_NAME}:${RCLONE_BUCKET_NAME}/backups/${timestamp}"
    fn_print_warning "将创建新的云端备份: ${timestamp}"
    ...
    for item in "${paths_to_sync[@]}"; do
        if [ -e "$ST_DIR/$item" ]; then
            fn_print_warning "正在上传: $item"
            if ! rclone sync "$ST_DIR/$item" "${remote_path}/${item}" --progress; then
                fn_print_error "上传 '$item' 失败！备份中止。"
                rclone purge "${remote_path}" # 清理不完整的备份
                sync_success=false
                break
            fi
        fi
    ...
    if $sync_success; then
        ...
        mapfile -t all_backups < <(rclone lsf "${RCLONE_REMOTE_NAME}:${RCLONE_BUCKET_NAME}/backups" --dirs-only | sort)
        if [ "${#all_backups[@]}" -gt $BACKUP_LIMIT ]; then
            local backups_to_delete_count=$(( ${#all_backups[@]} - BACKUP_LIMIT ))
            fn_print_warning "备份数量超过上限(${BACKUP_LIMIT})，将删除 ${backups_to_delete_count} 个最旧的备份。"
            for ((i=0; i<backups_to_delete_count; i++)); do
                local old_backup="${all_backups[$i]}"
                echo "  - 删除: ${old_backup}"
                rclone purge "${RCLONE_REMOTE_NAME}:${RCLONE_BUCKET_NAME}/backups/${old_backup}"
            done
...
rclone_restore() {
    clear; fn_print_header "Rclone从云端恢复数据 (下载)"
    if [ ! -f "$RCLONE_SYNC_CONFIG_FILE" ]; then fn_print_error "请先在菜单 [1] 中配置Rclone同步服务。"; fn_press_any_key; return; fi
    # shellcheck source=/dev/null
    source "$RCLONE_SYNC_CONFIG_FILE"

    fn_print_warning "正在获取云端备份列表..."
    mapfile -t backup_list < <(rclone lsf "${RCLONE_REMOTE_NAME}:${RCLONE_BUCKET_NAME}/backups" --dirs-only | sort -r)
    if [ ${#backup_list[@]} -eq 0 ]; then fn_print_error "未在云端找到任何备份。"; fn_press_any_key; return; fi

    echo "请选择要恢复的备份版本 (按时间倒序):"
    for i in "${!backup_list[@]}"; do
        printf "  [%-2d] %s\n" "$((i + 1))" "${backup_list[$i]%/}"
    done
    read -p "请输入选项 (其他键取消): " choice
    if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 1 ] || [ "$choice" -gt "${#backup_list[@]}" ]; then fn_print_warning "操作已取消。"; fn_press_any_key; return; fi
    
    local selected_backup="${backup_list[$((choice-1))]}"
    fn_print_warning "此操作将使用备份 [${selected_backup%/}] 【覆盖】本地数据！"
    read -p "确认要恢复吗？[y/N]: " confirm; if [[ ! "$confirm" =~ ^[yY]$ ]]; then fn_print_warning "操作已取消。"; fn_press_any_key; return; fi

    local remote_path="${RCLONE_REMOTE_NAME}:${RCLONE_BUCKET_NAME}/backups/${selected_backup}"
    local paths_to_sync=("data" "public/scripts/extensions/third-party" "plugins" "config.yaml")
    local sync_success=true
    for item in "${paths_to_sync[@]}"; do
        fn_print_warning "正在下载: $item"
        if ! rclone sync "${remote_path}${item}" "$ST_DIR/$item" --progress; then
            fn_print_error "下载 '$item' 失败！恢复中止。"; sync_success=false; break
        fi
    done
    if $sync_success; then fn_print_success "数据已从云端成功恢复！"; fi
    fn_press_any_key
}

rclone_clear_config() {
    if [ -f "$RCLONE_SYNC_CONFIG_FILE" ]; then
        read -p "确认要清除已保存的Rclone同步配置吗？(y/n): " confirm
        if [[ "$confirm" =~ ^[yY]$ ]]; then 
            # shellcheck source=/dev/null
            source "$RCLONE_SYNC_CONFIG_FILE"
            rclone config delete "$RCLONE_REMOTE_NAME"
            rm -f "$RCLONE_SYNC_CONFIG_FILE"
            fn_print_success "Rclone同步配置已清除。"; 
        else fn_print_warning "操作已取消。"; fi
    else fn_print_warning "未找到任何Rclone同步配置。"; fi
    fn_press_any_key
}
...
menu_rclone_sync() {
    ...
    while true; do
        clear; fn_print_header "数据同步 (Rclone/S3 方案)"
        echo -e "      [1] ${CYAN}配置Rclone同步服务${NC}"; echo -e "      [2] ${GREEN}备份到云端 (上传)${NC}"; echo -e "      [3] ${YELLOW}从云端恢复 (下载)${NC}"; echo -e "      [4] ${RED}清除Rclone同步配置${NC}"; echo -e "      [0] ${CYAN}返回上一级${NC}"
        read -p "    请输入选项: " choice
        case $choice in 1) rclone_configure ;; 2) rclone_backup ;; 3) rclone_restore ;; 4) rclone_clear_config ;; 0) break ;; *) fn_print_error "无效输入。"; sleep 1 ;; esac
    done
}
...
main_sync_menu() {
    while true; do
        clear; fn_print_header "数据同步方案选择"
        echo -e "      [1] ${CYAN}Git 方案 (适合技术用户，提供版本历史)${NC}"
        echo -e "      [2] ${GREEN}Rclone/S3 方案 (适合普通用户，类似网盘)${NC}"
        echo -e "      [0] ${CYAN}返回主菜单${NC}"
        read -p "    请输入选项: " choice
        case $choice in 1) menu_git_sync ;; 2) menu_rclone_sync ;; 0) break ;; *) fn_print_error "无效输入。"; sleep 1 ;; esac
    done
}
...
fn_create_data_zip_backup() {
    ...
    mkdir -p "$BACKUP_ROOT_DIR"; local timestamp; timestamp=$(date +"%Y-%m-%d_%H-%M"); local backup_name="ST_核心数据_${timestamp}.zip"; local backup_zip_path="${BACKUP_ROOT_DIR}/${backup_name}"
    cd "$ST_DIR" || { fn_print_error "无法进入 SillyTavern 目录进行备份。"; return 1; }
    local has_files=false; for item in "${paths_to_backup[@]}"; do if [ -e "$item" ]; then has_files=true; break; fi; done
    ...
    if zip -rq "$backup_zip_path" "${paths_to_backup[@]}" "${exclude_params[@]}"; then
        fn_print_success "核心数据备份成功: ${backup_name}"; cd "$HOME"; echo "$backup_zip_path"; return 0
    else
...
main_install() {
    ...
    if [[ "$auto_start" == "true" ]]; then
        ...
        yes | pkg upgrade -y; yes | pkg install git nodejs-lts rsync zip unzip termux-api coreutils gawk bc rclone || fn_print_error_exit "核心依赖安装失败！"
    ...
    if [ -f "$ST_DIR/start.sh" ]; then fn_print_warning "检测到完整的 SillyTavern 安装，跳过下载。"; elif [ -d "$ST_DIR" ] && [ -n "$(ls -A "$ST_DIR")" ]; then fn_print_error_exit "目录 $ST_DIR 已存在但安装不完整。请手动删除该目录后再试。"; else
        local download_success=false; mapfile -t sorted_mirrors < <(fn_find_fastest_mirror)
        if [ ${#sorted_mirrors[@]} -eq 0 ]; then fn_print_error_exit "所有 Git 镜像均测试失败，无法下载主程序。"; fi
...
main_update_st() {
    ...
    local update_success=false; mapfile -t sorted_mirrors < <(fn_find_fastest_mirror)
    if [ ${#sorted_mirrors[@]} -eq 0 ]; then fn_print_error "所有 Git 镜像均测试失败，无法更新。"; fn_press_any_key; return; fi
    for mirror_url in "${sorted_mirrors[@]}"; do
        local mirror_host; mirror_host=$(echo "$mirror_url" | sed -e 's|https://||' -e 's|/.*$||')
        fn_print_warning "正在尝试使用镜像 [${mirror_host}] 更新..."; git remote set-url origin "$mirror_url"
        ...
        if [ $? -eq 0 ]; then fn_print_success "代码更新成功。"; if fn_run_npm_install_with_retry; then update_success=true; fi; break; else
            if echo "$git_output" | grep -qE "overwritten by merge|Please commit|unmerged files"; then
                clear; fn_print_header "检测到更新冲突！"; fn_print_warning "原因: 你可能修改过酒馆的文件，导致无法自动合并新版本。"
                ...
                '1') fn_print_warning "正在执行强制覆盖 (git reset --hard)..."; if git reset --hard "origin/$REPO_BRANCH" && git pull origin "$REPO_BRANCH"; then fn_print_success "强制更新成功。"; if fn_run_npm_install_with_retry; then update_success=true; fi; else fn_print_error "强制更新失败！"; fi; break ;;
    ...
    if ! $update_success; then fn_print_error "已尝试所有可用线路，但更新均失败。"; fi
    fn_press_any_key
}
...
run_backup_interactive() {
    ...
    local exclude_params=(-x "*/_cache/*" -x "*.log" -x "*/backups/*"); zip -rq "$backup_zip_path" "${paths_to_backup[@]}" "${exclude_params[@]}"
    if [ $? -ne 0 ]; then fn_print_warning "备份失败！"; fn_press_any_key; return; fi
    mapfile -t all_backups < <(find "$BACKUP_ROOT_DIR" -maxdepth 1 -name "*.zip" -printf "%T@ %p\n" | sort -nr | cut -d' ' -f2-); fn_print_success "备份成功：${backup_name}.zip (当前备份数: ${#all_backups[@]}/${BACKUP_LIMIT})"; printf "%s\n" "${paths_to_backup[@]}" >"$CONFIG_FILE"
    if [ "${#all_backups[@]}" -gt $BACKUP_LIMIT ]; then
        echo -e "${YELLOW}备份数量超过上限，正在清理旧备份...${NC}"; local backups_to_delete=("${all_backups[@]:$BACKUP_LIMIT}")
        for old_backup in "${backups_to_delete[@]}"; do rm "$old_backup"; echo "  - 已删除: $(basename "$old_backup")"; done; fn_print_success "清理完成。"
    fi; fn_press_any_key
}

main_migration_guide() {
    clear; fn_print_header "数据迁移 / 恢复指南"; echo -e "${YELLOW}请遵循以下步骤，将您的数据从旧设备迁移到新设备，或恢复备份：${NC}"; echo -e "  1. 在旧设备上，用MT管理器等文件工具，进入目录：\n     ${CYAN}${BACKUP_ROOT_DIR}/${NC}"; echo -e "  2. 找到需要的备份压缩包 (例如 ST_核心数据_xxxx.zip)，并将其发送到新设备。"; echo -e "  3. 在新设备上，将这个压缩包移动或复制到 SillyTavern 的根目录：\n     ${CYAN}${ST_DIR}/${NC}"; echo -e "  4. 使用 MT 管理器等工具，将压缩包 ${GREEN}“解压到当前目录”${NC}。"; echo -e "  5. 如果提示文件已存在，请选择 ${YELLOW}“全部覆盖”${NC}。"; echo -e "  6. 操作完成后，重启 SillyTavern 即可看到所有数据。"; echo -e "\n${YELLOW}如需更详细的图文教程，请在主菜单选择 [8] 查看帮助文档。${NC}"; fn_press_any_key
}
...
run_delete_backup() {
    ...
    echo -e "检测到以下备份 (当前/上限: ${#backup_files[@]}/${BACKUP_LIMIT}):"; local i=0
    for file in "${backup_files[@]}"; do if [ -f "$file" ]; then local size; size=$(du -h "$file" | cut -f1); printf "    [%-2d] %-40s (%s)\n" "$((++i))" "$(basename "$file")" "$size"; fi; done
    read -p "输入要删除的备份编号 (其他键取消): " choice
    if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 1 ] || [ "$choice" -gt "${#backup_files[@]}" ]; then echo "操作已取消."; fn_press_any_key; return; fi
    local chosen_backup="${backup_files[$((choice - 1))]}"; read -p "确认删除 '$(basename "$chosen_backup")' 吗？(y/n): " confirm
    if [[ "$confirm" =~ ^[yY]$ ]]; then rm "$chosen_backup"; fn_print_success "备份已删除。"; else echo "操作已取消。"; fi; fn_press_any_key
}

main_data_management_menu() {
    while true; do
        clear; fn_print_header "SillyTavern 本地数据管理"; echo -e "      [1] ${GREEN}创建自定义备份${NC}"; echo -e "      [2] ${CYAN}数据迁移/恢复指南${NC}"; echo -e "      [3] ${RED}删除旧备份${NC}"; echo -e "      [0] ${CYAN}返回主菜单${NC}"; read -p "    请输入选项: " choice
        case $choice in 1) run_backup_interactive ;; 2) main_migration_guide ;; 3) run_delete_backup ;; 0) break ;; *) echo -e "${RED}无效输入。${NC}"; sleep 1 ;; esac
    done
}

main_update_script() {
    clear; fn_print_header "更新助手脚本"; echo -e "${YELLOW}正在从 Gitee 下载新版本...${NC}"; local temp_file; temp_file=$(mktemp)
...
check_for_updates_on_start() {
    ( local temp_file; temp_file=$(mktemp); if curl -L -s --connect-timeout 10 -o "$temp_file" "$SCRIPT_URL"; then if ! cmp -s "$SCRIPT_SELF_PATH" "$temp_file"; then touch "$UPDATE_FLAG_FILE"; else rm -f "$UPDATE_FLAG_FILE"; fi; fi; rm -f "$temp_file" ) &
}

fn_create_shortcut() {
    local BASHRC_FILE="$HOME/.bashrc"; local ALIAS_CMD="alias st='\"$SCRIPT_SELF_PATH\"'"; local ALIAS_COMMENT="# SillyTavern 助手快捷命令"
    if ! grep -qF "$ALIAS_CMD" "$BASHRC_FILE"; then chmod +x "$SCRIPT_SELF_PATH"; echo -e "\n$ALIAS_COMMENT\n$ALIAS_CMD" >>"$BASHRC_FILE"; fn_print_success "已创建快捷命令 'st'。请重启 Termux 或执行 'source ~/.bashrc' 生效。"; fi
}
...
main_manage_autostart() {
    ...
    if $is_set; then echo -e "当前状态: ${GREEN}已启用${NC}"; echo -e "${CYAN}提示: 关闭自启后，输入 'st' 命令即可手动启动助手。${NC}"; read -p "是否取消自启？ (y/n): " confirm; if [[ "$confirm" =~ ^[yY]$ ]]; then sed -i "/# SillyTavern 助手/d" "$BASHRC_FILE"; sed -i "\|$AUTOSTART_CMD|d" "$BASHRC_FILE"; fn_print_success "已取消自启。"; fi
    else echo -e "当前状态: ${RED}未启用${NC}"; echo -e "${CYAN}提示: 在 Termux 中输入 'st' 命令可以手动启动助手。${NC}"; read -p "是否设置自启？ (y/n): " confirm; if [[ "$confirm" =~ ^[yY]$ ]]; then echo -e "\n# SillyTavern 助手\n$AUTOSTART_CMD" >>"$BASHRC_FILE"; fn_print_success "已成功设置自启。"; fi; fi
    fn_press_any_key
}

main_open_docs() {
    clear; fn_print_header "查看帮助文档"; local docs_url="https://blog.qjyg.de"; echo -e "文档网址: ${CYAN}${docs_url}${NC}\n"
    if fn_check_command "termux-open-url"; then termux-open-url "$docs_url"; fn_print_success "已尝试在浏览器中打开，若未自动跳转请手动复制上方网址。"; else fn_print_warning "命令 'termux-open-url' 不存在。\n请先安装【Termux:API】应用及 'pkg install termux-api'。"; fi
    fn_press_any_key
}
...
EOF
    ...
    case $choice in
    1) main_start ;; 2) main_sync_menu ;; 3) main_data_management_menu ;; 4) main_install ;;
    5) main_update_st ;; 6) main_update_script ;; 7) main_manage_autostart ;; 8) main_open_docs ;;
